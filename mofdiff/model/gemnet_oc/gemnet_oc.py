...
from mofdiff.model.scaling.compat import load_scales_compat
...
class GemNetOC(torch.nn.Module):
    """
    Arguments
    ----------
    num_targets: int
    ...
    def __init__(
        ...
        **kwargs,  # backwards compatibility with deprecated arguments
        ...
        self.in_size_atom = in_size_atom
        self.atom_emb = AtomEmbedding(in_size_atom, num_elements)
        self.use_latent = use_latent
        if use_latent:
            self.atom_latent_emb = torch.nn.Linear(in_size_atom + latent_dim, emb_size_atom)
        else:
            self.atom_latent_emb = torch.nn.Linear(in_size_atom, emb_size_atom)
    ...
    def generate_local_struct_graph(self, data):
        device = data.frac_coords.device
        
        node_offsets = torch.cat([
            torch.zeros(1, dtype=torch.long, device=device), 
            data.num_atoms.cumsum(dim=0)])
        edge_offsets = torch.cat([
            torch.zeros(1, dtype=torch.long, device=device),
            data.num_bonds.cumsum(dim=0)])
        
        new_data_list = []
        for i_mof, mof in enumerate(data.to_data_list()):
            num_nodes = mof.num_atoms
            all_frac_coords = mof.frac_coords
            all_atom_types = mof.atom_types
            all_edges = data.edge_index[:, edge_offsets[i_mof]:edge_offsets[i_mof+1]] - node_offsets[i_mof]
            all_to_jimages = data.to_jimages[edge_offsets[i_mof]:edge_offsets[i_mof+1]]
            all_bb_index = torch.arange(int(mof.num_atoms)).to(device)
            for bb_idx, bb in enumerate(mof.pyg_mols):                
                local_vectors = (bb.local_vectors * self.cp_scaling) @ mof.R[bb_idx]
                cp_frac_coords = (bb.centroid + local_vectors)
                all_frac_coords = torch.cat([all_frac_coords, cp_frac_coords % 1], dim=0)
                
                cp_atom_types = (mof.atom_types[bb_idx] +
                                 self.axis_emb(torch.ones(local_vectors.shape[0], dtype=torch.long, device=device))) / 2
                all_atom_types = torch.cat([all_atom_types, cp_atom_types], dim=0)
                
                num_cps = bb.local_vectors.shape[0]
                cp_edges = torch.stack([torch.ones(num_cps, dtype=torch.long, device=device) * bb_idx,
                                        torch.arange(num_cps, dtype=torch.long, device=device) + num_nodes])
                reverse_cp_edges = torch.stack([cp_edges[1], cp_edges[0]])
                cp_edges = torch.cat([cp_edges, reverse_cp_edges], dim=1)
                
                cp_to_jimages = cp_frac_coords - cp_frac_coords % 1
                reverse_to_jimages = -cp_to_jimages
                all_edges = torch.cat([all_edges, cp_edges], dim=1)
                all_to_jimages = torch.cat([all_to_jimages, cp_to_jimages, reverse_to_jimages], dim=0)
                all_bb_index = torch.cat([all_bb_index, torch.ones(num_cps, dtype=torch.long, device=device) * bb_idx]) # this needs to be offseted by num_atoms
                num_nodes = num_nodes + num_cps
            is_cg_node = torch.zeros(num_nodes, dtype=torch.long, device=device)
            is_cg_node[torch.arange(int(mof.num_atoms)).to(device)] = 1
            all_cart_coords = frac_to_cart_coords(all_frac_coords, mof.lengths, mof.angles, num_nodes)
            num_bonds = (num_nodes - mof.num_atoms) * 2 + data.num_bonds[i_mof]
            atom_node = torch.zeros(num_nodes, dtype=torch.long, device=device)
            atom_node[:mof.num_atoms] = 1
            new_data_list.append(Data(
                z=mof.z,
                pos=all_cart_coords,
                frac_coords=all_frac_coords,
                atom_types=all_atom_types,
                # atom_types=is_cg_node, # we will inserst bb type embedding later. 
                num_atoms=num_nodes,
                natoms=num_nodes,
                lengths=mof.lengths,
                angles=mof.angles,
                ...
                to_jimages=all_to_jimages,
                is_cg_node=is_cg_node,
                num_components=mof.num_atoms,
                bb_index=all_bb_index,
                num_bonds=num_bonds,
                atom_node=atom_node,
            ))
    ...
    def generate_graph(self, data,
        ...
        if use_pbc:
            ...
            num_bonds = get_n_edge(edge_index[0], data.num_atoms).to(edge_index.device)
            ...
            if not self.use_pbc:
                neighbors = get_n_edge(edge_index[0], data.num_atoms).to(edge_index.device)
            else:
    ...
    def generate_graph_dict(self, data, cutoff, max_neighbors, otf_graph=True):
    ...
    def get_graphs_and_indices(self, data, otf_graph=True):
        """ "Generate embedding and interaction graphs and indices."""
        num_atoms = data.atom_types.size(0)
        ...
        if (
            ...
            a2a_graph = self.generate_graph_dict(
                data, self.cutoff_aint, self.max_neighbors_aint, otf_graph
            )
            ...
            main_graph = self.generate_graph_dict(
                data, self.cutoff, self.max_neighbors, otf_graph
            )
            a2a_graph = {}
            a2ee2a_graph = {}
        if self.quad_interaction:
    ...
    def forward(self, data, no_emb=False, output_R=False):
        ...
        (
            ...
            trip_idx_e2a,
            quad_idx,
        ) = self.get_graphs_and_indices(data, self.otf_graph)
        pos = data.pos if 'pos' in data else frac_to_cart_coords(
